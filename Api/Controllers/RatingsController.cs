...
namespace Api.Controllers
{
    ...
    public class RatingsController : ControllerBase
    {
        ...
        [HttpPost("ratings/")]
        public async Task<Dictionary<string, Gaussian>> GetRatings([FromBody] RatingsOptions options)
        {
            var result = await CalculateRatings(options);
            return result.ToDictionary(rating => rating.Key.ToString(), rating => new Gaussian(rating.Value.GetMean(), rating.Value.GetVariance()));
        }

        private async Task<Dictionary<int, Microsoft.ML.Probabilistic.Distributions.Gaussian>> CalculateRatings(RatingsOptions options)
        {
            ...
            var (playerSkill, _, _, _, _, _) = GGScore.GGScore.Infer(convertedMatches,
